...
export default function merge_sorted_trackers(
  limited_trackers = [],
  selfassesment_trackers = [],
  remove_records_with_future_deadlines = false
) {
  if (remove_records_with_future_deadlines) {
    limited_trackers = limited_trackers.filter(filter_remove_future_deadlines);
    selfassesment_trackers = selfassesment_trackers.filter(
      filter_remove_future_deadlines
    );
  }
  ...
  while (
    limited_index < limited_trackers.length &&
    selfassesment_index < selfassesment_trackers.length
  ) {
    let limited_tracker = limited_trackers[limited_index];
    let selfassesment_tracker = selfassesment_trackers[selfassesment_index];

    if (
      limited_tracker.fields.deadline > selfassesment_tracker.fields.deadline
    ) {
